...
if TYPE_CHECKING:
    from .config import BufferReader, FileSystem
...
class LazyItem:
    ...
    # noinspection SpellCheckingInspection
    def _chunked_readb(self, start: int, end: int):
        """
...
class LazyReader(LazyItem):
    def __init__(
        ...
        *,
        ...
        self._raw_stats = LazyRawStats(original_pos, sep_c, toc_start, toc_size)
    ...
    def raw_data(self, chunked: bool = True):
        """
        Use to directly read the raw bytes out as a generator or a bytes stream.

        If `chunked=True`, returns a generator that can be consumed.
        If `chunked=False`, returns the whole bytes stream.
        ...
        For a combined archive (generated by combing several archives together),
        this method extract the target child archive out, more or less like a reverse of combining.

        """
        if not chunked:
            return self._readb(
                -self._raw_stats.header_size,
    ...
    def msgpack_raw_data(self, chunked: bool = True):
        ...
        """
        if not chunked:
            return self._readb(0, self._raw_stats.data_size)

        return self._chunked_readb(0, self._raw_stats.data_size)
