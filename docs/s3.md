...
s3 = S3FileSystem()
with s3.open('my-bucket/my-file', 'rb') as f:
...
s3 = S3FileSystem()
with LazyReader('my-bucket/my-file', fs=s3) as reader:
    ...
```
...
s3 = S3FileSystem()
with LazyWriter('my-bucket/my-file', fs=s3) as writer:
    writer.write(...)
```
...
with LazyWriter(object_name, fs=fs) as writer:
    writer.write({'hello': 'world'})

with LazyReader(object_name, fs=fs) as reader:
    print(reader.to_obj())
```

## More FS Support

In fact, any `fsspec` compatible file system objects can be passed in.
The following is an example using the `pyarrow` package.
...
Because `pyarrow` supports other file systems, they can be wrapped and passed in as well.
